...
export default function Register({ onLogin }) {
  ...
  const [form, setForm] = useState({ name: 'Ada Okafor', email: 'ada@jalalhub.com', password: 'password123' });
    ...
    <div className="min-h-screen bg-[linear-gradient(135deg,_#f7fff9_0%,_#ffffff_100%)] px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-6xl flex-col overflow-hidden rounded-[36px] border border-green-100 bg-white shadow-2xl lg:flex-row">
        <div className="flex-1 bg-gradient-to-br from-green-600 to-green-800 p-8 text-white sm:p-10">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-green-100">Join the platform</p>
          <h1 className="mt-3 text-3xl font-semibold sm:text-4xl">Open a secure account and start paying bills instantly.</h1>
          <p className="mt-4 text-sm leading-7 text-green-50">Create your wallet, manage services and enjoy one-click utility payments.</p>
        </div>
        <div className="flex-1 p-8 sm:p-10">
          <div className="mb-6">
            <p className="text-sm font-semibold text-green-600">Create account</p>
            <h2 className="text-2xl font-semibold text-slate-900">Register</h2>
          </div>
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div>
              <label className="mb-2 block text-sm font-medium text-slate-700">Full name</label>
              <input className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-green-500" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} />
            </div>
            <div>
            ...
            <button className="w-full rounded-2xl bg-green-600 px-4 py-3 font-semibold text-white">Create account</button>
          </form>
          <p className="mt-6 text-sm text-slate-600">Already have an account? <Link to="/login" className="font-semibold text-green-700">Login here</Link></p>
        </div>
      </div>
    </div>
  );
